...
function getMovies(searchTerm, page) {
  ...
  fetch(url)
    ...
    .then(({Search: movies = [], totalResults = 0}) => {
      const totalPages = Math.ceil(totalResults /10); //math.ceil rundet. wir machen das um die pages unten anzuzeigen
                        ...
                        <div class="row row-cols-2 g-4">
                            ...
                            <table class="table">
                              ...
                              <tbody>
                                ...
                                <ul>
                                  <p> ${data.Ratings.map( rating => `<li ${rating.Value} </li></li>`).join('')}  </p>
                                </ul>
                            </div>
                        </div>
                        <p>${data.Plot}</p>
